...
**Domain:** Multi-tenant per-user dockerized AI coding agent runner (web platform)
**Researched:** 2026-04-11
**Confidence:** HIGH for Go/Next/Stripe/Postgres core, MEDIUM for terminal-streaming approach, MEDIUM for sandbox-isolation tier

---

## TL;DR

Mirror MSV's Go core almost verbatim — Echo v4 + pgx + zerolog is the right call and saves weeks. Diverge from MSV in three places: **(1) drop Temporal** (overkill for one-active-session-per-user; a small Go worker pool + Postgres-backed job table is enough), **(2) add the Docker Engine SDK directly** (`github.com/moby/moby/client`) instead of shelling out, and **(3) add `ttyd` running inside each user container** as the web-terminal backend, fronted by a Go WebSocket reverse proxy that does the auth check. The chat UI talks to the same container via a thin Go-side stdio bridge to the agent CLI process — *not* a second container.

For sandbox isolation on a shared Hetzner host running untrusted user code, plain Docker is **insufficient long-term**; plan for **Sysbox** (drop-in runtime, no app changes) as the v1.5 hardening step. gVisor/firecracker are out of scope for v1.
...
**Decision: Do NOT pull in Temporal yet** (MSV uses it; it's overkill here).
...
If you want a tiny library instead of hand-rolling: `github.com/riverqueue/river` (Postgres-backed Go queue, type-safe, well-maintained as of 2026). Optional.
...
```bash
# Initialize
go mod init github.com/<org>/agent-playground/api
...
go get -t github.com/pashagolub/pgxmock/v4@v4.9.0
go get -t github.com/alicebob/miniredis/v2@v2.37.0
```
...
sudo tee /etc/docker/daemon.json <<'JSON'
{
  ...
  "default-ulimits": { "nofile": { "Soft": 4096, "Hard": 8192 } },
  "live-restore": true
}
JSON
...
**If a user is on the platform-billed credits tier:**
...
**If the agent is from a curated recipe (`agents/<name>/`):**
...
**If the agent is bootstrapped (generic Claude-Code path):**
...
1. **Single Go binary** for the API (not microservices). Subpackages: `api/`, `auth/`, `sessions/`, `containers/`, `billing/`, `recipes/`, `proxy/` (LLM egress proxy), `terminal/` (ttyd reverse proxy).
2. **Postgres is the single source of truth.** Redis is a cache only — losing Redis is a brownout, not an outage.
3. **One Hetzner box.** No multi-host until you have the user count to justify it. The Go orchestrator owns the host's Docker daemon directly.
4. **Two WebSocket endpoints per session:**
   - `/api/sessions/:id/terminal` → proxies to in-container ttyd (xterm.js direct or iframe)
   - `/api/sessions/:id/chat` → proxies stdin/stdout of the agent CLI process (chat UI)
5. **One HTTP egress proxy per session** (unix socket inside the container) for platform-billed metering.
6. **Recipes are code, not config.** Each `agents/<name>/` folder has a `Dockerfile` + `recipe.yaml` + a Go test that boots it.
7. **Sysbox runtime ready from day 1, enabled in v1.5.** Build the recipe images so they work under both `runc` and `sysbox-runc` (don't rely on host devices, don't need `--privileged`).
...
1. **ttyd integration mode: iframe vs direct attach via WebSocket?** Iframe is 1-day work, direct attach is 1-week work but tighter UX. Recommendation: **iframe for v1, direct attach for v2.**
2. **LLM egress proxy as a sidecar container vs an in-process Go HTTP server bound to a per-session unix socket inside the user container?** Unix socket is simpler, no networking. Sidecar gives cleaner isolation. **Recommendation: unix socket via a bind-mounted file from a host-side socket directory.** Lighter, easier to debug.
3. **Should the Go orchestrator hold the agent process directly (for chat) or should each container expose a small HTTP "agent driver" that the orchestrator talks to?** First is faster to ship; second is the right long-term shape. **Recommendation: ship #1, plan #2.**
4. **License: MIT vs Apache-2.0?** Recommendation: **Apache-2.0** — explicit patent grant matters for an OSS platform that may attract corporate contributors.
5. **Image registry: ghcr.io vs self-hosted on the Hetzner box?** Recommendation: **ghcr.io** in v1 — free for public images, zero ops.
...
*Stack research for: multi-tenant per-user dockerized AI coding agent platform*
*Researched: 2026-04-11*
