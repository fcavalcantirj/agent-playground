...
### 📍 RESUME ANCHOR — READ AFTER /clear (UPDATED 2026-04-30 post-22c.3-07-execution)

**Phase 22c.3 (in-app chat channel) Wave 2 COMPLETE.** Plan 22c.3-07 (inapp_outbox pump — 100ms tick PG → Redis Pub/Sub) committed 2026-04-30 (3 commits: d00e459 deps+fixtures, 59445ce RED tests, 65a4989 GREEN module). **8 testcontainer-PG + testcontainer-Redis integration tests PASS in 9.77s**: happy path (3 unpublished → 3 publishes + UPDATE), skip-published (no re-publish), **D-35 abandon-after-1h** (2h-old row filtered out, stays published=false), **Pitfall 3 strategy 2 Redis-error rollback** (mock-patched `redis.publish` raises on 2nd call → 0 swept; retry tick PASSes all 3), **D-32 SKIP LOCKED via asyncio.gather** of 2 `_pump_once` calls (no double-publish across 5 rows), **D-09 per-agent fan-out** (agent A receives 2; agent B receives 1), stop_event responsive cancel <1s, **D-34 envelope shape** (exact 5-key set + types). First transactional-outbox pattern in api_server (PATTERNS.md GREENFIELD). 31/31 adjacent integration regressions (Plans 04 + 05 + 06) green; 88/88 unit-test regressions green. testcontainers[redis] dev extra added; `redis_container` (session) + `redis_client` (per-test) conftest fixtures. One Rule-3 deviation: pyproject `redis>=5.2,<7` runtime pin bumped to `<8` because `testcontainers[redis]>=4.14.2` requires `redis>=7` (redis 7.x asyncio API source-compatible with 5.x). **Wave 3 is next: Plan 22c.3-08 (routes — POST/SSE GET/DELETE on `/v1/agents/:id/messages`) and Plan 22c.3-09 (lifespan — wires the 3 background tasks + Redis client + httpx.AsyncClient + restart sweep + image rebuild + live deploy).**
...
Phase 22c.3  — In-App Chat            🟡 EXECUTING — Plans 01 + 02 + 03 + 04 + 05 + 06 + 07 SHIPPED; Wave 2 COMPLETE; next is Wave 3 (Plan 22c.3-08 routes + Plan 22c.3-09 lifespan)
...
Why: Plan 07 closed the data-plane infrastructure trio (dispatcher Plan 05 + reaper Plan 06 + outbox pump Plan 07). All three asyncio tasks consume `state.db` and the dispatcher/outbox additionally consume `state.redis`/`state.bot_http_client`. Plan 22c.3-09 lifespan attaches them at app boot with their respective stop_events; Plan 22c.3-08 routes layer (POST/SSE/DELETE on `/v1/agents/:id/messages`) calls into the same store seam Plans 04..07 share. Wave 5 e2e gates phase exit. picoclaw is OUT of scope.
