...
status: "Phase 22c.3 EXECUTING — Wave 4 COMPLETE 2026-04-30 night. Plan 22c.3-14 (nullclaw channels.inapp — FINAL Wave-4 recipe opt-in) SHIPPED (commits 4913547 + 6bdb13b). recipes/nullclaw.yaml +129 lines: added channels.inapp block (transport=http_localhost / port=3000 / contract=a2a_jsonrpc / endpoint=/a2a / method=POST / auth_mode=none) + request_envelope (JSON-RPC 2.0 message/send shape mirroring dispatcher's hardcoded envelope at services/inapp_dispatcher.py:168-180) + response_envelope (reply_path=$.result.artifacts[0].parts[0].text + task_id_path + state_path) + streaming (transport=sse / method=message/stream / delta_field=append / done_field=lastChunk) + health_endpoint=/health + ready_log_regex='Gateway listening on' + persistent_argv_override (sh -c heredoc-writes /nullclaw-data/config.json with a2a.enabled=true + gateway.require_pairing=false + gateway.host=0.0.0.0 + allow_public_bind=true + agent identity, then `exec nullclaw gateway --host 0.0.0.0 --port 3000`) + activation_env (OPENROUTER_API_KEY=${INAPP_PROVIDER_KEY} + INAPP_AGENT_NAME={agent_name} + INAPP_AGENT_URL={agent_url}) + verified_cells[0] citing spikes/recipe-nullclaw.md (revised v3 native A2A) 2026-04-30 FULL_PASS. Round-3 supersession discipline honored: ZERO sidecar/python3/apk add/nohup/INAPP_BOT_BIN tokens anywhere in channels.inapp; the native nullclaw gateway IS the bridge. Live api_server restarted via `docker compose -f docker-compose.prod.yml -f docker-compose.local.yml restart api_server`; /healthz=200 within ~5s; GET /v1/recipes shows channels_supported=['telegram', 'inapp'] for nullclaw; live container `docker exec deploy-api_server-1 python3 -c '...InappRecipeIndex...'` returns InappChannelConfig(contract='a2a_jsonrpc', port=3000, endpoint='/a2a', auth_mode='none'). One Rule-3 deviation auto-fixed inline (plan verify cmd `tools/run_recipe.py validate` typo — actual flag is `--lint`; substituted equivalent direct YAML parse + InappRecipeIndex programmatic check + live registry projection per Plan 13 precedent). channels.telegram UNTOUCHED (D-20 additive); recipes/picoclaw.yaml UNCHANGED (deferred per user direction 2026-04-30, backward compat with smoke suite preserved). Fifth and FINAL Wave-4 recipe opt-in completing the 5-recipe matrix: hermes (22c.3-10 openai_compat) + nanobot (22c.3-11 openai_compat) + openclaw (22c.3-12 openai_compat) + zeroclaw (22c.3-13 zeroclaw_native) + nullclaw (22c.3-14 a2a_jsonrpc). LOUD GAP FLAGGED for Plan 22c.3-15: routes/agent_lifecycle.py::start_persistent does NOT yet consume channels.inapp.persistent_argv_override at deploy time, NOT yet mint per-session INAPP_AUTH_TOKEN, NOT yet substitute ${INAPP_AUTH_TOKEN}/${INAPP_PROVIDER_KEY}/{agent_name}/{agent_url} placeholders — without this runner extension, Wave 5 e2e gate cannot pass. Flag now restated in 5 successive plan SUMMARIES (10/11/12/13/14)."
stopped_at: "2026-04-30 — 22c.3-14 SHIPPED (commits 4913547 + 6bdb13b); FIFTH and FINAL Wave-4 recipe opt-in (nullclaw via native Google A2A JSON-RPC 2.0 + a2a_jsonrpc contract + Round-3 supersession of v2 sidecar pattern). 5-recipe matrix COMPLETE. Wave 4 CLOSED. Next: /gsd-execute-phase 22c.3 → Plan 22c.3-15 (e2e gate + runner-side wiring extension to consume persistent_argv_override + mint INAPP_AUTH_TOKEN)."
last_updated: "2026-04-30T21:53:00.000Z"
progress:
  total_phases: 19
  completed_phases: 5
  total_plans: 33
...
### 📍 RESUME ANCHOR — READ AFTER /clear (UPDATED 2026-04-30 post-22c.3-14-execution)

**Phase 22c.3 (in-app chat channel) Wave 4 COMPLETE.** Plan 22c.3-14 (nullclaw channels.inapp — FIFTH and FINAL Wave-4 recipe opt-in via native Google A2A JSON-RPC 2.0) committed 2026-04-30 (2 commits: 4913547 channels.inapp block, 6bdb13b verified_cells[0]). **recipes/nullclaw.yaml +129 lines** — channels.inapp block with `transport=http_localhost`, `port=3000`, `contract=a2a_jsonrpc`, `endpoint=/a2a`, `auth_mode=none` + native A2A request/response/streaming envelopes + persistent_argv_override (sh -c heredoc-writes /nullclaw-data/config.json with `a2a.enabled=true` + `gateway.require_pairing=false` + `gateway.host=0.0.0.0` + `allow_public_bind=true` + agent identity, then `exec nullclaw gateway --host 0.0.0.0 --port 3000`) + activation_env (OPENROUTER_API_KEY/INAPP_AGENT_NAME/INAPP_AGENT_URL) + verified_cells[0] citing spikes/recipe-nullclaw.md (revised v3 native A2A) FULL_PASS. **Round-3 supersession discipline honored**: ZERO sidecar/python3/apk add/nohup/INAPP_BOT_BIN tokens anywhere in channels.inapp; the native nullclaw gateway IS the bridge. Live api_server restarted; `/healthz=200`; `GET /v1/recipes` shows `channels_supported=['telegram', 'inapp']` for nullclaw; live container `InappRecipeIndex.get_inapp_block('nullclaw')` returns `InappChannelConfig(contract='a2a_jsonrpc', port=3000, endpoint='/a2a', auth_mode='none')`. One Rule-3 deviation auto-fixed inline (plan verify cmd `tools/run_recipe.py validate` typo — actual flag is `--lint`; substituted equivalent direct YAML parse + InappRecipeIndex programmatic check + live registry projection per Plan 13 precedent). channels.telegram UNTOUCHED (D-20 additive); recipes/picoclaw.yaml UNCHANGED.

**5-recipe matrix COMPLETE:** hermes (22c.3-10 openai_compat /v1/chat/completions :8642) + nanobot (22c.3-11 openai_compat /v1/chat/completions :8900) + openclaw (22c.3-12 openai_compat /v1/chat/completions :18789) + zeroclaw (22c.3-13 zeroclaw_native /webhook :42617) + nullclaw (22c.3-14 a2a_jsonrpc /a2a :3000). picoclaw remains DEFERRED per user direction 2026-04-30.

**LOUD GAP FLAGGED for Plan 22c.3-15** (restated in 5 successive plan SUMMARIES — 10/11/12/13/14): `routes/agent_lifecycle.py::start_persistent` does NOT yet consume `channels.inapp.persistent_argv_override` at deploy time, NOT yet mint per-session `INAPP_AUTH_TOKEN`, NOT yet substitute `${INAPP_AUTH_TOKEN}` / `${INAPP_PROVIDER_KEY}` / `{agent_name}` / `{agent_url}` placeholders. Without this runner extension, Wave 5 e2e gate cannot pass for any of the 5 recipes (nanobot might pass partially with auth_mode=none but the entrypoint swap is still required). Plan 22c.3-15 must include this runner extension as part of its exit gate or spawn a sub-plan that lands the wiring before the e2e flow is exercised.
...
Phase 22c.3  — In-App Chat            🟡 EXECUTING — Plans 01..14 SHIPPED; Wave 4 COMPLETE (5/5 recipes opted into inapp: hermes + nanobot + openclaw + zeroclaw + nullclaw); next is Plan 22c.3-15 (Wave 5 e2e gate + runner-side wiring extension)
...
Why: Wave 4 is now CLOSED. All 5 recipes ship channels.inapp on the v0.2 schema with empirically-validated contracts. The only remaining work is Plan 22c.3-15 — Wave 5 e2e gate that exercises the full POST /v1/agents/:id/messages → dispatcher → contract adapter → bot endpoint → reply → SSE stream path against a live container. Plan 22c.3-15 must ALSO include (or be preceded by) the runner-side wiring extension flagged in 5 successive plan SUMMARIES. picoclaw is OUT of scope (deferred).
