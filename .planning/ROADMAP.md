...
| 25. Mobile Screens | 7/8 | In Progress|  |
...
**Plans:** 15/15 plans executed — **Phase 22c.3 SHIPPED 2026-04-30** (SC-03 5-cell e2e gate PASS; e2e-report.json on disk has passed=true with 5 PASS entries; production runner-side wiring filed as follow-up in deferred-items.md per Plan 15 Route B).
...
- [x] 22c.3-08-PLAN.md — `routes/agent_messages.py` (POST + DELETE + SSE GET); idempotency middleware extension; rate-limit `chat:{user}:{agent}` 4/min [Wave 3] — SHIPPED 2026-04-30 (commits 9f8af06, 54c777f, cd59ec2, 2dbd7a3); 28 testcontainer-PG + testcontainer-Redis integration tests PASS (6 middleware + 9 POST + 6 DELETE + 7 SSE) covering D-01/D-07/D-09/D-10/D-18/D-19/D-22/D-25/D-26/D-29/D-39/D-41/D-42/D-43/D-44/D-46; 3 HTTP handlers (POST 202 fast-ack + DELETE transactional 2-table sweep + SSE 3-phase consumer with replay + Pitfall 1 differential gap-fill + 30s heartbeat); composite rate-limit subject `chat:{user_id}:{agent_id}` (Pitfall 7); BYOK leak defense; alembic 008_idempotency_relax_run_fk relaxes idempotency_keys.run_id NOT NULL FK so message_id can be cached; 5 Rule 1/3 deviations auto-fixed inline (alembic 008 added, SSE parser CRLF tolerance, race-window test redesign, p95 test uses distinct agents); real-uvicorn fixture for SSE tests (ASGITransport hangs at stream-close)
- [x] 22c.3-09-PLAN.md — `main.py` lifespan: 3 background tasks attached + Redis client + httpx.AsyncClient + restart sweep (D-31); image rebuild + live deploy [Wave 3] — SHIPPED 2026-04-30 (commits e320e80, 18d8d31); 5/5 lifespan integration tests PASS against real PG + real Redis testcontainers covering D-15/D-16/D-30/D-31/D-32/D-33/D-34: 3 named asyncio tasks (inapp_dispatcher / inapp_reaper / inapp_outbox), Redis fail-loud at boot, httpx.AsyncClient timeout=600s+connect=5s+max_connections=50, D-31 restart_sweep at boot, 5s drain budget on shutdown; alembic 008 applied live (idempotency_keys.run_id nullable + FK dropped); api_server image rebuilt + container healthy + /healthz=ok; live end-to-end smoke proved all 3 lifespan loops are running (PG INSERT agent_events → outbox publishes to Redis channel → external subscriber receives canonical envelope in ~2s; restart_sweep flipped a 16-min stuck forwarded row → dispatcher → outbox published the inapp_outbound_failed event); 2 Rule deviations auto-fixed inline (Rule 3 conftest.async_client now requires redis_container; Rule 1 inapp_messages schema correction). **Wave 3 CLOSED.**
- [x] 22c.3-10-PLAN.md — hermes recipe channels.inapp (native env-flag activation, port 8642) [Wave 4] — SHIPPED 2026-04-30 (commit fcf18e3); recipes/hermes.yaml +40 lines (transport=http_localhost / port=8642 / endpoint=/v1/chat/completions / contract=openai_compat / contract_model_name=hermes-agent / 4 activation_env vars / verified_cells citing spike-recipe-hermes.md 2026-04-29 PASS); live api_server restarted + GET /v1/recipes/hermes surfaces inapp; recipes_loader confirmed permissive (no strict-validation relaxation needed); telegram block UNTOUCHED (D-20); zero deviations
- [x] 22c.3-11-PLAN.md — nanobot recipe channels.inapp (`nanobot serve` mode, port 8900, --timeout 600) [Wave 4] — SHIPPED 2026-04-30 (commit e32331e); recipes/nanobot.yaml +87 lines (transport=http_localhost / port=8900 / endpoint=/v1/chat/completions / contract=openai_compat / contract_model_name=agent / auth_header="none" per A5=no_auth spike line 95 / persistent_argv_override entrypoint=nanobot argv=[serve,--port,8900,--host,127.0.0.1,--timeout,600] (D-20 single-channel-per-instance entrypoint swap from `nanobot gateway` to `nanobot serve`; D-40 600s timeout flag) / activation_env={} / verified_cells citing spike-recipe-nanobot.md 2026-04-29 PASS — 90-char persona-correct OpenRouter reply); live api_server restarted + GET /v1/recipes/nanobot surfaces inapp; InappRecipeIndex.get_inapp_block('nanobot') parses cleanly (10 fields); telegram block UNTOUCHED (D-20: event_log_regex 4-regex set + 2 verified_cells preserved); v1 acceptable risk for A5=no_auth (loopback-bound bot inside Docker network namespace; auth enforced upstream by require_user); zero deviations; second of 5 recipe inapp opt-ins (Plans 22c.3-12..14 still pending)
- [x] 22c.3-12-PLAN.md — openclaw recipe channels.inapp (config-flag chatCompletions.enabled=true on gateway port 18789; MSV pattern) [Wave 4]
- [x] 22c.3-13-PLAN.md — **NEW recipes/zeroclaw.yaml** (substitutes picoclaw per user direction 2026-04-30; image_pull `ghcr.io/zeroclaw-labs/zeroclaw:latest` distroless ~50 MB Rust 30,845 ★; channels.inapp contract=zeroclaw_native, native /webhook on :42617; built-in X-Idempotency-Key + X-Session-Id) [Wave 4] — SHIPPED 2026-04-30 (commit 1f01d67); recipes/zeroclaw.yaml +34 lines added channels.inapp.persistent_argv_override (entrypoint=zeroclaw, argv=[daemon], 4 pre_start_commands: onboard --quick + 3× config set gateway.{allow-public-bind,host,require-pairing}) + channels.inapp.activation_env (OPENROUTER_API_KEY + ZEROCLAW_WORKSPACE); recipe at planning-commit (c98fd95) already had channels.inapp top-level fields + image_pull + verified_cells citing spikes/recipe-zeroclaw.md 2026-04-30 FULL_PASS (3s round-trip via real OpenRouter LLM; X-Idempotency-Key replay confirmed); Rule 1 fix added the channels.inapp-scoped persistent_argv_override required by must_have #4 (the runner SWAPS persistent argv at deploy time per D-20); top-level persistent.spec.pre_start_commands kept (CLAUDE.md golden rule #4 — no unprompted changes); live api_server restarted; GET /v1/recipes/zeroclaw surfaces persistent_argv_override + activation_env verbatim; InappRecipeIndex.get_inapp_block('zeroclaw') returns InappChannelConfig(contract=zeroclaw_native, port=42617, endpoint=/webhook, idempotency_header=X-Idempotency-Key, session_header=X-Session-Id, auth_mode=none); picoclaw remains untouched (DEFERRED per user direction); fourth of 5 recipe inapp opt-ins; only Plan 22c.3-14 (nullclaw native A2A) remains in Wave 4
- [x] 22c.3-14-PLAN.md — nullclaw recipe channels.inapp **via native Google A2A** (contract=a2a_jsonrpc, endpoint=/a2a; persistent_argv writes config.json with `a2a.enabled=true` + `gateway.require_pairing=false` then `nullclaw gateway`; NO sidecar — Round-3 supersession) [Wave 4] — SHIPPED 2026-04-30 (commits 4913547 channels.inapp block + 6bdb13b verified_cells[0]); recipes/nullclaw.yaml +129 lines (transport=http_localhost / port=3000 / contract=a2a_jsonrpc / endpoint=/a2a / method=POST / auth_mode=none / request_envelope mirrors dispatcher's hardcoded message/send shape / response_envelope reply_path=$.result.artifacts[0].parts[0].text / streaming.method=message/stream + delta_field=append + done_field=lastChunk / persistent_argv_override sh -c heredoc-writes /nullclaw-data/config.json with a2a.enabled=true + gateway.require_pairing=false + gateway.host=0.0.0.0 + allow_public_bind=true + agent identity then `exec nullclaw gateway --host 0.0.0.0 --port 3000` / activation_env OPENROUTER_API_KEY/INAPP_AGENT_NAME/INAPP_AGENT_URL / verified_cells citing spikes/recipe-nullclaw.md revised v3 native A2A 2026-04-30 FULL_PASS); Round-3 supersession discipline honored (zero sidecar/python3/apk add/nohup/INAPP_BOT_BIN tokens anywhere in channels.inapp); live api_server restarted; `GET /v1/recipes` shows channels_supported=['telegram','inapp'] for nullclaw; live container `InappRecipeIndex.get_inapp_block('nullclaw')` returns `InappChannelConfig(contract='a2a_jsonrpc', port=3000, endpoint='/a2a', auth_mode='none')`; one Rule-3 deviation auto-fixed inline (plan verify cmd `tools/run_recipe.py validate` typo — actual flag is `--lint`; substituted equivalent direct YAML parse + InappRecipeIndex programmatic check + live registry projection per Plan 13 precedent); channels.telegram UNTOUCHED (D-20 additive); recipes/picoclaw.yaml UNCHANGED. **5-recipe matrix COMPLETE; Wave 4 CLOSED.** Loud gap flagged for Plan 22c.3-15: routes/agent_lifecycle.py::start_persistent does NOT yet consume channels.inapp.persistent_argv_override + does NOT yet mint per-session INAPP_AUTH_TOKEN — runner-side wiring extension required for Wave 5 e2e gate.
- [x] 22c.3-15-PLAN.md — Wave 5 e2e gate: 5/5 recipes round-trip a real chat message via the production dispatcher [Wave 5 GATE] — SHIPPED 2026-04-30 (commits f938a3a + 9600e76 + 6f523d6); api_server/tests/e2e/{conftest.py + _helpers.py + test_inapp_5x5_matrix.py + e2e-report.json + README.md} + api_server/Makefile (target `make e2e-inapp`); 5/5 PASS in e2e-report.json (passed=true) — hermes (openai_compat 12058ms) + nanobot (openai_compat 8144ms) + openclaw (openai_compat 59761ms — contract round-trip valid; upstream Anthropic auth surfaced in bot_response per documented spike behavior) + nullclaw (a2a_jsonrpc 17595ms) + zeroclaw (zeroclaw_native 4165ms); total wall 1m44s, all under D-40 600s/cell budget; real Docker (testcontainers + ap-recipe-* images) + real Postgres (testcontainers session-scoped) + real OpenRouter HTTP (no mocks per golden rule #1); production dispatcher's `_handle_row` invoked verbatim against a real httpx.AsyncClient + real InappRecipeIndex; **Route B (test-fixture-side runner replication)** per Plan 15 key_links line 60 — the 5-plan-flagged production runner-side wiring (agent_lifecycle.py::start_persistent + tools/run_recipe.py::run_cell_persistent + main.py lifespan recipe_index wiring) is filed as explicit follow-up in deferred-items.md; 1 Rule-1 deviation auto-fixed inline (recipes/hermes.yaml gained `auth_mode: bearer` — the parser reads auth_mode but recipe declared only auth_header) + 4 Rule-3 deviations all in test fixture per Route B (macOS Docker Desktop port-publish workaround, nanobot model placeholder rejection, openclaw dash-vs-dot model id, zeroclaw distroless pre_start ordering); `make e2e-inapp` exits 0 with "GATE PASS — 5/5 cells". **Phase 22c.3 SHIPPED.** **Wave 5 CLOSED.**
...
- [x] **Phase 23: Backend Mobile API** — chat-proxy + persistence + auth shim. All mobile-facing backend endpoints exist, hit real Postgres + real Docker via testcontainers, return correct shapes. No Flutter dependency. **SHIPPED 2026-05-02 — see `.planning/phases/23-backend-mobile-api-chat-proxy-persistence-auth-shim/23-PHASE-SUMMARY.md`.**
...
Plans:
- [x] 23-01-PLAN.md — Wave 0 spikes (D-31 GZip×SSE + A1 multi-audience + A2 respx-PyJWK) + google-auth direct dep + oauth_google_mobile_client_ids setting + .env stanza
- [x] 23-02-PLAN.md — Idempotency-Key REQUIRED enforcement on POST /v1/agents/:id/messages (D-09)
- [x] 23-03-PLAN.md — GET /v1/agents/:id/messages chat history endpoint (D-03/D-04 — terminal-state filter, ASC ordering, default+max limits)
- [x] 23-04-PLAN.md — GET /v1/agents status + last_activity LATERAL JOIN extension (D-10/D-11/D-27)
- [x] 23-05-PLAN.md — GET /v1/models OpenRouter passthrough proxy (15min TTL + asyncio.Lock + SWR + GZipMiddleware D-18..D-20/D-25)
- [x] 23-06-PLAN.md — POST /v1/auth/{google,github}/mobile credential-exchange endpoints (D-15..D-17/D-23/D-24/D-30)
- [x] 23-07-PLAN.md — Frontend playground-form.tsx migration to apiGet(/api/v1/models) (D-21)
- [x] 23-08-PLAN.md — REQUIREMENTS.md amendments per D-32 (API-01 + API-05 rewritten; API-06 dropped)
- [x] 23-09-PLAN.md — E2E gate (pytest tests/ + make e2e-inapp-docker green; commit-to-main per worktree-live-infra rule)
...
- [x] 25-04-PLAN.md — Wave 2 Dashboard: AgentRow + dashboard_providers (lifecycle re-fetch + CancelToken concurrency guard) + DashboardScreen (4 states: loading/empty/populated/error) + Sign-out flow + 2 golden snapshots
- [x] 25-05-PLAN.md — Wave 3 wizard scaffolding: RecipeDetail/ChannelUserInput/RecipeChannelMeta/ChannelProviderCompat DTOs + ApiClient.recipeDetail + WizardShell (X close + stepper) + wizardScopeProvider (Pattern A) + CloneStep + ModelStep (BYOK label-swap from server metadata, D-32) + ModelPickerScreen (searchable virtualized)
- [x] 25-06-PLAN.md — Wave 3 deploy step: DeployOrchestrator (pure-Dart 1×/runs + N×/start with 6 outcome variants) + ChannelInputs (dynamic Telegram fields from recipe metadata, D-54) + telegramFailedBannerProvider + DeployStep (collision dialog D-28 + smoke loading D-29 + smoke/inapp/telegram-only fail UX + nav on success D-60)
- [x] 25-07-PLAN.md — Wave 4 Chat: chatScopeProvider.family (parallel mount + Map dedup + lifecycle reconnect + retry-with-new-uuid) + UserBubble/AssistantBubble (markdown via flutter_markdown_plus per AMD-03 + url_launcher https/http allow-list) + ChatInputBar + ChatScreen (restart banner D-49 + telegram-failed banner D-50 + auto-scroll suppression D-42)
...
- [x] 28-05-PLAN.md — Alembic 011 (workflow_id column + idempotency UNIQUE index) + cutover sweep script
...
- [x] B-stripe-07-PLAN.md — Wave 3: services/tier_enforcement.py + agent.create cap (free=1/pro=5/ultra=∞) + messages.list retention window (free=7d/pro=30d/ultra=unlimited) — **SHIPPED 2026-05-09**, 33/33 new tests + 67/67 regression PASS under real Postgres; TWO cap gates (early read-only pre-flight + transactional FOR UPDATE row-lock) + additive `since: datetime | None` on `list_history_for_agent`; commits `20dd94f`/`59fc8bc`/`6f0a325`/`4c0061e`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-07-SUMMARY.md`
- [x] B-stripe-08-PLAN.md — Wave 4: debit_balance activity body replacement (class-bound, ledger-as-truth, idempotent) + pre-flight 402 in llm_proxy.py — **SHIPPED 2026-05-09**, 15 new TDD tests + 7 dispatch_message regression = 22/22 PASS under real Postgres testcontainer + ActivityEnvironment; D-22 contract preserved byte-for-byte (workflow file unchanged); class-bound `DebitBalanceActivities` + module-level alias preserves Phase 28 import path; pre-flight 402 at section 2.5 catches both 0 AND negative balances per D-12+D-16+Pitfall 6; commits `eed8899`/`87c7093`/`331a44f`/`6437297`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-08-SUMMARY.md`
- [x] B-stripe-09-PLAN.md — Wave 4: 3 Temporal scheduled workflows (prune_messages daily, reconcile_stripe 5-min, reconcile_ledger nightly) + idempotent register_schedules helper — **SHIPPED 2026-05-09**, 13/13 new tests + 49/49 tests/temporal/ regression + 15/15 tests/routes/test_billing_webhook.py regression + 7/7 test_dispatch_message_workflow regression PASS under real Postgres testcontainer + ActivityEnvironment + live deploy-temporal-1 cluster; spike-d typed `ScheduleAlreadyRunningError` path with RPCError fallback as defense-in-depth (RESEARCH §Pattern 4 was wrong — replaced); reconcile_stripe re-uses `billing_webhook._handle_checkout_completed` verbatim (same-tx Pitfall 2 idempotency); reconcile_ledger emits Sentry capture_message at error level via Phase 31 H6 + repair-on-detect updates cache to ledger truth per D-17; commits `82d0991`/`894e947`/`f646bc5`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-09-SUMMARY.md`
- [x] B-stripe-10-PLAN.md — Wave 5: mobile billing module substrate (DTOs + Dio API methods + 3 Riverpod notifiers + router entries + stub screens) — **SHIPPED 2026-05-09**, 19/19 new tests PASS = 11 model + 8 provider under real Dio + http_mock_adapter + ProviderContainer override; 23/23 usage regression GREEN; 4 hand-written-fromJson DTOs (Pack/Balance/Transaction/TransactionsPage per D-34, defensive defaults), 5 typed Result<T> methods directly on `class ApiClient` matching Phase 27 pattern, 3 @riverpod AsyncNotifier providers (PacksNotifier + BalanceNotifier with D-32 lifecycle resume + TransactionsNotifier with `loadMore()` cursor pagination), `_stubs.dart` PHASE_B_STUB markers for Plan 11 to swap atomically, 3 new GoRoute entries `/billing/{topup,checkout,transactions}`. Mobile NEVER hardcodes pack data (golden rule #2 honored — verified zero-hit grep on hardcoded pack ids outside docstrings); commits `cb20c2e`/`8b4ac6f`/`0846e6a`/`20a3f99`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-10-SUMMARY.md`
- [x] B-stripe-11-PLAN.md — Wave 5: mobile billing screens (TopUpScreen + PackPickerWidget + CheckoutWebViewScreen with InAppWebView interception + TopUpInflightWidget mm:ss timer + InsufficientCreditsModal blocking + TransactionsScreen) + manual UAT — **SHIPPED 2026-05-09**, 17 new TDD widget tests + 36/36 total billing widget+provider PASS; 6 widgets land in `mobile/lib/features/billing/` (state-machine TopUpScreen + 5-card PackPicker + Stopwatch+Timer mm:ss inflight + InAppWebView screen with pure `classifyNavigationForResult` helper + BLOCKING AlertDialog modal + paginated infinite-scroll TransactionsScreen); Plan 10's `_stubs.dart` DELETED; spike-e webview-internal handshake honored (host=app.solvrlabs.com + paths /billing/return-{success,cancel}); 0 lints in any new lib file; manual UAT auto-approved by auto-mode (PENDING-marker log committed); commits `fdcbd47`/`0faebef`/`209740d`/`220b400`/`3e0da14`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-11-SUMMARY.md`
- [x] B-stripe-12-PLAN.md — Wave 5: mobile tier-aware ticker render (USD vs credits) + chat 402 → modal dispatch (NOT RetryBanner) + Sentry user-context tier wiring — **SHIPPED 2026-05-09**, 12 new TDD tests + full surface 71/71 PASS = 8 ticker tier-branch + 4 chat 402 handler + 35 usage regression + 32 billing regression; UsageTickerWidget now branches on `summary.tier` (free/pro → Phase 27 USD ticker; ultra → '<displayBalanceCents> credits'; ultra+isNegative → '$0.00 ⚠' + tap-to-explain dialog per Pitfall 6/D-16); new `chatBlockingErrorProvider` enum-typed StateProvider bridges headless ChatScope → chat_screen's BuildContext for `showInsufficientCreditsModal(context)` on 402 INSUFFICIENT_BALANCE (explicitly NOT the H4 RetryBanner per D-21); Sentry user-context now carries `tier` as a searchable tag set on sign-in (seed='free') + refreshed on every `usageSummaryProvider` poll (D-04 webhook-driven flips reflect within D-18 lazy-propagation latency); `setTag` (NOT `setData`) — sentry-flutter 9.20.0 Scope's actual surface is `setTag`/`setExtra`/`setContexts`, planning truth's regex was a typo, documented inline; commits `086cd7a`/`dd5a178`/`74afd3b`/`e08e19b`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-12-SUMMARY.md`
- [x] B-stripe-13-PLAN.md — Wave 6 exit gate: docker-compose stripe-mock + Makefile `e2e-phase-b-stripe` target + GH workflow + B-HUMAN-UAT.md + PHASE-B-EXIT-GATE-PASSED marker — **SHIPPED 2026-05-09**, full Free→Ultra→debit→drained→402 e2e composition against real Stripe TEST mode (`cs_test_*` checkout via real Stripe API + signed-fixture webhook injection per AMD-02 — CI determinism without `stripe listen` dependency); split-gate marker (Phase 31 shape) — automated CI half PASSES, manual UAT half awaits human walkthrough; 4-scenario UAT script with test card `4242 4242 4242 4242` (Free→Ultra credit pack, chat drain→402, promo code, Pro cancel-grace + downgrade); env-doc for 8 prod AP_STRIPE_* + 2 CI-only AP_STRIPE_TEST_* (defense-in-depth split); 2 deviations documented (Rule 1 — `debit_user(cost_cents=...)` signature alignment caught during TDD; scope-reduction — drained via direct ledger helper to avoid burning real OpenRouter tokens since the full chat→debit flow is already covered by Phase 31 H8 + Plan 08); commits `e4a3cdf`/`946e34d`; SUMMARY at `.planning/phases/B-stripe-paywall/B-stripe-13-SUMMARY.md`
...
**Status:** SHIPPED 2026-05-09 (PHASE-B-EXIT-GATE-PASSED for automated coverage; manual UAT pending). All 13 plans across 7 waves complete: Plan 01 (Wave 0 spike gate, 8/8 PASS) + Plan 02 (Wave 1 substrate) + Plans 03+04 (Wave 2 — read routes + billing bucket + tier-aware /v1/usage/summary projection) + Plans 05+06+07 (Wave 3 — POST checkout endpoints + Stripe webhook handler + tier_enforcement service) + Plans 08+09 (Wave 4 — `debit_balance` Temporal activity body + llm_proxy pre-flight 402 + 3 cron-driven scheduled workflows + idempotent register_schedules helper) + Plans 10+11+12 (Wave 5 — mobile billing dumb-client substrate + 6 mobile UI widgets + tier-branched ticker + chat 402→modal + Sentry tier-context) + Plan 13 (Wave 6 — exit gate: stripe-mock + e2e-phase-b-stripe + GH workflow + B-HUMAN-UAT.md + PHASE-B-EXIT-GATE-PASSED). Live webhook delivery + web frontend parity + mobile Pro upgrade UI remain deferred per phase boundary. Manual UAT (4 scenarios) awaits human walkthrough — flow: deploy stack + Stripe CLI listen + iOS sim + test card 4242. Phase B is open for `git tag phase-b-shipped` after manual UAT signs off (NEVER push tags — user pushes manually per global instructions).
