...
4. **Two curated agent recipes, thinly built on `ap-base`:**
   ...
   - **Hermes** (Python 3.13, Nous Research, MIT, TUI-first, six execution backends, multi-channel daemon) — the architecturally hardest agent. If it fits, the pattern generalizes. Pinned to a commit SHA.
...
- **D-10:** **Runtime deps in `ap-base`:** `tini`, `tmux`, `ttyd`, `git`, `curl`, `jq`, `ca-certificates`, `gosu`, `bash`. Recipe overlays add language runtimes (Node.js for OpenClaw, Python 3.13 + uv for Hermes, pinned-SHA Go binary for picoclaw).
...
- **D-19:** **Hermes is dockerized in Phase 2 despite its complexity** because it is the architecturally hardest agent and validates the substrate against TUI, multi-backend, and multi-channel daemon patterns. Source of truth: `github.com/NousResearch/hermes-agent`, MIT license, Python 3.13 baseline, `uv pip install -e ".[all,dev]"`, config dir `~/.hermes/`, `cli-config.yaml.example` in repo root as a template. **Note (corrected at research time):** Originally written as Python 3.11 in this CONTEXT. Verified via Hermes upstream Dockerfile which uses `ghcr.io/astral-sh/uv:0.11.6-python3.13-trixie` — the correct baseline is Python 3.13. All downstream plans use 3.13. ROADMAP SC-4 corrected to match (2026-04-14).
...
- `github.com/NousResearch/hermes-agent` — Hermes source; `pyproject.toml` for Python 3.13 + `uv` install path; `cli-config.yaml.example` for the template to pre-populate; `scripts/install.sh` for the install recipe; `mcp_serve.py` worth investigating for the chat bridge decision
...
- ~~**ROADMAP.md update** — reflect this reshape (Phase 2 scope shrinks; new hardening phase inserts before Phase 8; success criteria for Phase 2 rewrite to match).~~ **DONE** at research time (2026-04-13). Subsequent edits at revision time (2026-04-14): SC-4 corrected from Python 3.11 → 3.13 to match Hermes upstream; Phase 2 plan list updated to reflect 6-plan structure (Plan 04 split into 04+05; old 05 → 06).
