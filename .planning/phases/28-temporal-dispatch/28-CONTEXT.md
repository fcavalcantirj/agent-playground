...
- **D-08:** **Workflow ID = `msg-{user_id}-{message_uuid}`** (user-scoped using `inapp_messages.user_id` + the existing `inapp_messages.id`). MSV uses `msg-{telegramID}-{unixMillis}` for the same reason — user scoping in the ID itself. AP includes user_id for: (a) **observability** — search Temporal UI for `msg-{user_id}-*` to see all of a user's workflows in one query; (b) **defense-in-depth** — if a bug ever lets a workflow start for the wrong user, the workflow_id encodes the audited user_id and detection is instant; (c) consistency with MSV's pattern. ~30 extra bytes per ID, trivial cost. `WorkflowIDReusePolicy.REJECT_DUPLICATE` so the same (user_id, message_uuid) pair can't kick off two workflows. (Revised 2026-05-05 from `msg-{message_uuid}` after user feedback.)
