...
2. **Dumb client, intelligence in the API — applies to EVERY client.** Web (Next.js/React), mobile (Flutter), CLI, SDK, future clients — **NONE** of them query Postgres directly, talk to Redis directly, read recipe YAMLs directly, or hardcode catalogs the server owns. Every client is a thin terminal over the API. Recipes come from `GET /v1/recipes`, models from the API, costs/usage/aggregates from the API. **No `const defaultClones`/model arrays in React state, no hardcoded model lists in Flutter, no `SELECT *` from anywhere but api_server.** If a client needs data, it calls an HTTP endpoint api_server owns. Aggregations (sums, daily series, top-N) are computed server-side and returned ready-to-render — clients never `SUM`, never `GROUP BY`, never reach across the API to compute something the server should compute. UI actions round-trip — a button that flips local state without `await apiPost(...)` is theater and ships nothing. See `memory/feedback_dumb_client_no_mocks.md`.
